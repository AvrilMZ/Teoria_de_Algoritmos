...
def colorear_rec(grafo, vertices, indice_actual, n, visitados):
    ...
    for color in range(n):
        if all(
            visitado_color != color
            for v, visitado_color in visitados
            if v in grafo.adyacentes(vertice)
        ):  # Por cada vertice en visitados, si el vertice es vecino y el color asignado es distinto al actual
