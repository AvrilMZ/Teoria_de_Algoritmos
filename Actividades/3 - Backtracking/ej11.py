...
def sub_conj(lista, combinaciones, posibilidad, num, sum_actual, i_actual):
	...
	combinaciones = []
	for i in range(i_actual, len(lista)): # evito recorrer elementos ya vistos
		if sum_actual + lista[i] <= num:
			posibilidad.append(lista[i])
			sol = sub_conj(lista, combinaciones, posibilidad, num, sum_actual + lista[i], i + 1)
