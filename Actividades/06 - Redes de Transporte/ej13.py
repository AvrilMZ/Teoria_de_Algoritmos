...
def carlos(grafo_ciudad, casa, escuela, cant_hijos):
    ...
    if (
        sum_salida < cant_hijos
    ):  # no hay suficientes calles para que salgan todos los hijos
        return None

    for ficticio in ficticios:
        esquina, otra = ficticios[ficticio]
        if (
            flujo_max[(esquina, otra)] > 0 and flujo_max[(esquina, ficticio)] > 0
        ):  # para evitar que pasen por una misma calle, fuerzo a que directamente no puedan pasar
